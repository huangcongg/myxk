...
class Attach extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('attach_url, ctime', 'required'),
			array('ctime', 'numerical', 'integerOnly'=>true),
			array('attach_url', 'length', 'max'=>200),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('attach_id, attach_url, ctime', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'reportVideos' => array(self::HAS_MANY, 'ReportVideo', 'attach_id'),
		);
	}
